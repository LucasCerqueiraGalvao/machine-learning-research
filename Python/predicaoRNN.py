...
# Definir o número de timesteps para a RNN (aumentado significativamente)
timesteps = 200  # Você pode ajustar este valor conforme o tamanho do seu conjunto de dados

# Criar sequências para a RNN
X = []
y = []
...
# Construir o modelo RNN com um número significativamente maior de neurônios
rnn_model = Sequential()
rnn_model.add(SimpleRNN(units=500, activation='relu', return_sequences=True, input_shape=(X_train.shape[1], X_train.shape[2])))
rnn_model.add(SimpleRNN(units=500, activation='relu'))
...
for _ in range(dias_futuros):
    ...
    # Fazer a previsão
    pred_scaled = rnn_model.predict(input_seq_reshaped)
